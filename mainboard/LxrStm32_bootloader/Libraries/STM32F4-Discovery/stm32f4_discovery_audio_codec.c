...
static uint32_t Codec_Init(uint16_t OutputDevice, uint8_t Volume, uint32_t AudioFreq)
{
  ...
  counter += Codec_WriteRegister(0x05, 0x20);
  

  //disable limiter
  Codec_WriteRegister(0x28, 0x00);
  ...
  if (CurrAudioInterface != AUDIO_INTERFACE_DAC)
  {  
    /* Disable the digital soft ramp and disable deemphasis*/
	  // 7 passthru b
	  // 6 passthru a
	  // 5 passb mute
	  // 4 passa mute
    ...
    counter += Codec_WriteRegister(0x0E, 0x07); // emphasis, soft ramp + zero crossing on
  }
  /* Disable the limiter attack level */
  counter += Codec_WriteRegister(0x27, 0x00);

  /* Disable tone control JS*/
  Codec_WriteRegister(0x1e, 0x38);
  //00 beep off
  //1 beep mix off
  //11 treble auf 15kHz
  //00 bass auf 50Hz
