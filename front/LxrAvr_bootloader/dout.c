...
void dout_updateOutputs()
{
	...
	uint16_t steps = dout_outputData[3] | (dout_outputData[4]<<8);
	steps++;// = steps << 1;
	if(steps==0)steps=1;
	dout_outputData[3] = steps & 0xff;
	dout_outputData[4] = steps>>8;
	/*
	dout_outputData[3] =  (dout_outputData[3]<<1);
	if(dout_outputData[3]==0x00)
	{
		dout_outputData[4] = 1 | (dout_outputData[4]<<1);
	}
	*/
#if USE_BRIGHTNESS
