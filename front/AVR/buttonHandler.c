...
void buttonHandler_buttonReleased(uint8_t buttonNr) {
	...
	case BUT_SHIFT:
		...
		if (buttonHandler_getMode() == SELECT_MODE_VOICE) {
			...
			led_clearSelectLeds();
			led_setValue(1,
					(uint8_t) (menu_getViewedPattern() + LED_PART_SELECT1));
			menu_switchPage(EUKLID_PAGE);
	...
	case BUT_SELECT8:
		...
		if (copyClear_Mode >= MODE_COPY_PATTERN) {
			...
			if (buttonHandler_getShift() && buttonHandler_stateMemory.selectButtonMode == SELECT_MODE_VOICE) {
				buttonHandler_handleSelectButton(
						(uint8_t) (buttonNr - BUT_SELECT1));
			}
		}
...
void buttonHandler_buttonPressed(uint8_t buttonNr) {
	...
	case BUT_SELECT8:
		...
		if (copyClear_Mode >= MODE_COPY_PATTERN) {
			...
			if (copyClear_srcSet()) {
				//select dest
				copyClear_setDst((int8_t) (buttonNr - BUT_SELECT1),
						MODE_COPY_PATTERN);
				...
				//select src
				copyClear_setSrc((int8_t) (buttonNr - BUT_SELECT1),
						MODE_COPY_PATTERN);
	...
	case BUT_VOICE_7:
		...
		if (copyClear_Mode >= MODE_COPY_PATTERN) {
			...
			if (copyClear_srcSet()) {
				//select dest
				copyClear_setDst((int8_t) (buttonNr - BUT_VOICE_1),
						MODE_COPY_TRACK);
				copyClear_copyTrack();
				led_clearAllBlinkLeds();
				//query current sequencer step states and light up the corresponding leds 
				...
				//select src
				copyClear_setSrc((int8_t) (buttonNr - BUT_VOICE_1),
						MODE_COPY_TRACK);
				led_setBlinkLed((uint8_t) (LED_VOICE1 + buttonNr - BUT_VOICE_1),
						1);
			}
		} else {
...
case BUT_SHIFT:
	...
	if (buttonHandler_getMode() == SELECT_MODE_VOICE) {
		buttonHandler_enterSeqMode();
			...
			|| (buttonHandler_getMode() == SELECT_MODE_PAT_GEN)) {
		...
		led_setBlinkLed((uint8_t) (LED_PART_SELECT1 + menu_getViewedPattern()),
				1);
	} else if (buttonHandler_getMode() == SELECT_MODE_STEP) {
